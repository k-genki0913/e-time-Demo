...
public class Monthly_Report_DAO {
	...
	public List<Monthly_Report_DTO> getList(List<Integer> monthly_report_id_list){
		Connection con = null;
		...
		String sql = "SELECT monthly_report_id, target_year, target_month, user_id, workingDay, total_working_hours, "
				...
				+ "FROM Monthly_Report WHERE monthly_report_id = ?";
		
		try {
			con = BaseDAO.getConnection();
			pstmt = con.prepareStatement(sql);
			
			for(Integer monthly_report_id: monthly_report_id_list) {
				...
				if(rs.next()) {
					...
					int workingDay = rs.getInt("workingDay");
	...
	public Monthly_Report_DTO getMonthly_Report(Integer monthly_report_id) {
		Connection con = null;
		PreparedStatement pstmt = null;
		
		Monthly_Report_DTO monthly_report_DTO = null;
		...
		try {
			...
			if(rs.next()) {
				...
				Date updated_date = rs.getDate("updated_date");
				String updated_user = rs.getString("updated_user");
				monthly_report_DTO = new Monthly_Report_DTO(monthly_report_id, target_year, target_month, record_user_id, 
							workingDay, total_working_hours, total_overtime_hours, work_on_a_day, totalWork_on_a_day_Hour, totalWork_on_a_day_OverTime,
							updated_date, updated_user);
			}
		} catch(SQLException e) {
			System.out.println("Monthly_Reportテーブルからの情報の取得に失敗しました");
			e.printStackTrace();
		} finally {
			if(pstmt != null) {
				try {
