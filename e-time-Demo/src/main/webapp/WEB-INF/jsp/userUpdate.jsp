...
<c:if test="${not empty userUpdateErrorMsg }">
	<p><c:out value="${userUpdateErrorMsg }" /></p>
</c:if>
...
◆メールアドレス：<input type="email" name="mail_address" value="${users_DTO.mail_address }" required><br>
◆管理者権限：<br>
<c:choose>
	<c:when test="${users_DTO.admin == 1 }" >
		<input type="radio" name="admin" value="1" required checked>有効<br>
	</c:when>
	<c:otherwise>
...
<c:choose>
	<c:when test="${users_DTO.admin == 0 }" >
		<input type="radio" name="admin" value="0" required checked>無効<br>
	</c:when>
	<c:otherwise>
...
<c:choose>
	<c:when test="${users_DTO.is_valid == 1 }" >
		<input type="radio" name="is_valid" value="1" required checked>有効<br>
	</c:when>
	<c:otherwise>
...
<c:choose>
	<c:when test="${users_DTO.is_valid == 0 }" >
		<input type="radio" name="is_valid" value="0" required checked>無効<br>
	</c:when>
	<c:otherwise>
...
<c:forEach var="role" items="${roleList}">
	<c:choose>
		<c:when test="${role.key == user_role_DTO.role_id }">
...
<c:forEach var="department" items="${departmentList}">
	<c:choose>
		<c:when test="${department.key == user_department_DTO.department_id }">
